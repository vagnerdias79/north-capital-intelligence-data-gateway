...
const baselineLinkFields = Object.freeze([
...
export function isLedgerBaseline(row, expectedFingerprint) {
  ...
  return row.ledger_fingerprint === expectedFingerprint
    && baselineLinkFields.every(field => metadata[field] !== undefined && metadata[field] !== null);
}
...
export function differences(original, baseline) {
  return comparableFields.flatMap(field => same(original[field], baseline[field])
    ? []
