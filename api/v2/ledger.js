...
const LEDGER_FINGERPRINT = 'NCI-LEDGER-AEF25E9D3A64';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!appUser) return json(res, 403, { ok:false, error:'IDENTITY_NOT_LINKED' });

    const portfolios = await sql`
      select id, code, name, base_currency, status, baseline_version
      from portfolios
      where user_id = ${appUser.id} and code = 'USD-INTL'
      limit 1
    `;
    ...
    const snapshots = await sql`
      ...
      where portfolio_id = ${portfolio.id}
        ...
        and fingerprint = ${LEDGER_FINGERPRINT}
    ...
    const rows = await sql`
      select
        t.id, t.trade_date, t.transaction_type, a.symbol as ticker,
        t.quantity, t.price, t.gross_amount, t.currency, t.fees,
        t.fx_rate, t.external_ref, t.source, t.notes, t.metadata
      from transactions t
      left join assets a on a.id = t.asset_id
      where t.portfolio_id = ${portfolio.id}
        and t.metadata ->> 'ledgerFingerprint' = ${LEDGER_FINGERPRINT}
      order by t.trade_date asc, t.created_at asc, t.id asc
    `;

    const counts = rows.reduce((acc,row)=>{
      const type=String(row.transaction_type||'UNKNOWN').toUpperCase();
    ...
    return json(res, 200, {
      ...
      mode:'READ_ONLY',
      authenticated:true,
      identity:{ subject:appUser.auth_subject, email:appUser.email, displayName:appUser.display_name },
      portfolio:{
      ...
      ledgerBaseline:{
        ...
        authoritativeForHistory:true,
        costBasisDerivedFromLedger:false
      },
      transactions:rows,
      count:rows.length,
      typeCounts:counts,
      nullAssetRows:rows.filter(x=>!x.ticker).length,
    ...
    return json(res,status,{ ok:false, error:error?.code||'LEDGER_READ_FAILED', message:status===500?'Unable to read migrated ledger.':error.message });
  }
}
